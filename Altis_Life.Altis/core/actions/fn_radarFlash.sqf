...
if (_speed > _limit) then {
	if (side _driver == civilian) then {
		...
		if(license_civ_driver OR license_civ_truck ) then {
			...
			if(life_speedTicket <= 3) then {systemChat format["Sie wurden %1 mal beim zu schnellen fahren erwischt",life_speedTicket];};
			if(life_speedTicket == 4) then {systemChat format["Sie wurden %1 mal beim zu schnellen fahren erwischt",life_speedTicket];};
			if(life_speedTicket > 4 && life_speedTicket < 8) exitWith {
					
					vehicle _driver setFuel 0.5;
			...
			if(life_cash <= _ticket) then
			{
				...
				[0] call SOCK_fnc_updatePartial;
					...
					if(life_speedCaught <= 5) then { systemChat format["Sie wurden %1 mal beim zu schnellen fahren ohne Führerschein erwischt",life_speedCaught];};
					if(life_speedCaught == 6) then { systemChat format["Sie wurden %1 mal beim zu schnellen fahren ohne Führerschein erwischt.",life_speedCaught];};
					
					if(life_speedTicket > 6 && life_speedTicket < 8) exitWith {
